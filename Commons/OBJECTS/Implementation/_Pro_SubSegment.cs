...
{
    [Serializable]
    public class _Pro_SubSegment : _ProjectObjectInfo
    {
        public _Pro_SubSegment(_COBJECTS p_Info)
            : base(p_Info)
        {
        ...
        public override decimal ZJFHJ
        {
            get
            {
                Decimal infos = (from i in this.Parent.StructSource.ModelProject.AsEnumerable()
                                 where Convert.ToInt32(i["PID"]) > 1

                                 select (i["UnitProject"] as _UnitProject).Reveal.ProSubSegment.ZJFHJ).Sum();
                
                /*Decimal infos = (from i in this.Parent.StrustObject.ObjectList.Values.Cast<_Engineering>()
