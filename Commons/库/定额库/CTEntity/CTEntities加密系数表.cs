...
namespace GOLDSOFT.QDJJ.COMMONS.LIB
{
	...
	/// </summary>
	[Serializable]
	public class CTEntities加密系数表 : CTEntities
	{
		/// <summary>
		///记录索引的值(私有成员)默认为-1
		/// </summary>
		...
		/// 成员实体(避免重复取索引)
		/// </summary>
		private CEntity加密系数表 m_CEntity加密系数表;
		/// <summary>
		///构造函数
		/// </summary>
		public CTEntities加密系数表()
		{
			this.buliderTable();
		}
		/// <summary>
		///反序列化构造函数
		/// </summary>
		public CTEntities加密系数表(SerializationInfo info, StreamingContext context):base(info, context)
		{
		}
		/// <summary>
		/// 获取当前集合指定行的实体对象
		...
		public CEntity加密系数表 this[int index]
		{
			get
			{
				...
				if (index == this.m_index) return this.m_CEntity加密系数表;
		...
		public int AppendEntityInfo(CEntity加密系数表 entity)
		{
			...
			if (entity != null)
			{
				...
				row[CEntity加密系数表.FILED_CAIJBH] = entity.CAIJBH;
				row[CEntity加密系数表.FILED_JMXS] = entity.JMXS;
				this.Rows.Add(row);
		...
		{
			this.Columns.Add(CEntity加密系数表.FILED_CAIJBH, typeof(System.String));
			this.Columns.Add(CEntity加密系数表.FILED_JMXS, typeof(System.String));
		}
	}
}
