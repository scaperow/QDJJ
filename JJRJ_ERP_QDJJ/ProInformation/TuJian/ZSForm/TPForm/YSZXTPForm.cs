...
{
    public partial class YSZXTPForm : BaseUI
    {
        public YSZXTPForm()
        {
            InitializeComponent();
        }
        public YSZXTPForm(_UnitProject p_CUnitProject)
            : base(p_CUnitProject)
        {
            InitializeComponent();
        }

        private void YSZXTPForm_Load(object sender, EventArgs e)
        {
            OnlyOneDataSource();
        }
        ...
        {
            this.bindingSource1.DataSource = InfTable.YSZXTP;
            this.InfTable.YSZXTP.RowChanged += new DataRowChangeEventHandler(this.RowChanged);
        ...
        private void ScreenWDBH(bool isAdd)
        {
            try
            {
                ...
                if (isAdd)
                {
                    ...
                    #region 确定清单
                    //（室外工程清单）
                    string strQDWhere = string.Format("TPLB = '艺术造型天棚'");
                    ...
                    //龙骨规格、材质
                    this.QDDEBindingSource.Filter = string.Format("TPLB='艺术造型天棚' and TPFL='龙骨'  and GGCZ = '{0}'", drCurrent["LGGGCZ"]);
                    ...
                    //基层规格、材质
                    this.QDDEBindingSource.Filter = string.Format("TPLB='艺术造型天棚' and TPFL='基层'   and GGCZ = '{0}'", drCurrent["JCGGCZ"]);
                    foreach (DataRowView item in this.QDDEBindingSource)
                    {
                        DataRow row = APP.UnInformation.DETable.NewRow();
                        row["DEBH"] = item["DEBH"];
                    ...
                    //面层规格、材质
                    this.QDDEBindingSource.Filter = string.Format("TPLB='艺术造型天棚' and TPFL='面层'   and GGCZ = '{0}'", drCurrent["MCGGCZ"]);
                    foreach (DataRowView item in this.QDDEBindingSource)
                    {
                        DataRow row = APP.UnInformation.DETable.NewRow();
                        row["DEBH"] = item["DEBH"];
        ...
        private void gridView1_CustomRowCellEditForEditing(object sender, DevExpress.XtraGrid.Views.Grid.CustomRowCellEditEventArgs e)
        {
            ...
            switch (e.Column.FieldName)
            {
                case "LGGGCZ":
                    ...
                    QDDEBindingSource.Filter = "TPLB='艺术造型天棚'  and TPFL ='龙骨'";
                ...
                case "JCGGCZ":
                    ...
                    //基层规格、材质
                    popControl1.DataSource = this.QDDEBindingSource;
                    QDDEBindingSource.Filter = "TPLB='艺术造型天棚' and TPFL ='基层'";
                ...
                case "MCGGCZ":
                    ...
                    //面层规格、材质
                    popControl1.DataSource = this.QDDEBindingSource;
                    QDDEBindingSource.Filter = "TPLB='艺术造型天棚' and TPFL ='面层'";
