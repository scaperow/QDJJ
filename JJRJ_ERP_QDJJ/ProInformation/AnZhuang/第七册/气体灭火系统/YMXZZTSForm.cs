...
{
    public partial class YMXZZTSForm : BaseUI
    {
        public YMXZZTSForm()
        {
            InitializeComponent();
        ...
        private void YMXZZTSForm_Load(object sender, EventArgs e)
        {
            OnlyOneDataSource();
        }
        ...
        {
            this.bindingSource1.DataSource = InfTable.严密性装置调试;
            this.InfTable.严密性装置调试.RowChanged += new DataRowChangeEventHandler(this.RowChanged);
            ...
            try
            {
                ...
                if (isAdd)
                {
                    ...
                    strString.Append(string.IsNullOrEmpty(toString(drCurrent["MC"])) ? " and  FL is null" : string.Format(" and  FL = '{0}'", drCurrent["MC"]))
                             .Append(string.IsNullOrEmpty(toString(drCurrent["SYRQGG"])) ? " and  GGXH is null" : string.Format("  and GGXH = '{0}'", drCurrent["SYRQGG"]));
        ...
        protected override void btnRefreshQDMC_Click(object sender, EventArgs e)
        {
            ...
            {
                strContent += "\r\n" + (++i) + ".气体灭火系统装置调试名称：" + drCurrent["MC"];
            }
            if (!string.IsNullOrEmpty(toString(drCurrent["SYRQGG"])))
            {
                strContent += "\r\n" + (++i) + ".规格、型号：" + drCurrent["SYRQGG"];
            }
        ...
        private void gridView1_CustomRowCellEditForEditing(object sender, DevExpress.XtraGrid.Views.Grid.CustomRowCellEditEventArgs e)
        {
            ...
            switch (e.Column.FieldName)
            {
                case "MC":
                    ...
                    this.XTZZTSBindingSource.Filter = " XT='气体灭火' and FL is not null";
                    //字符串截断并去重复
                    popControl1.DataSource = RemoveRepeat(this.XTZZTSBindingSource, "FL");

                    popControl1.ColName = new string[] { "名称|FL|MC" };
                    popControl1.RemoveDefaultColName = new string[] { "SYRQGG"};
                    popControl1.bind();
                    break;
                case "SYRQGG":
                    ...
                    popControl1.ColName = new string[] { "试验容器规格|GGXH|SYRQGG" };
                    popControl1.bind();
                    break;
            }
        }
        ...
        private void checkeArr()
        {
            ...
            if (currRow != null)
            {
                ...
                if (0 < XTZZTSBindingSource.Count)
                {
                    ...
                    {
                        checkMess.Add("试验容器规格");
                        CheckColl.Add("SYRQGG");
                    }
                }
