...
{
    public partial class SMHGDAZForm : BaseUI
    {
        public SMHGDAZForm()
        {
            InitializeComponent();
        ...
        private void SMHGDAZForm_Load(object sender, EventArgs e)
        {
            OnlyOneDataSource();
        }
        ...
        public override object Parm
        {
            ...
            set
            {
                ...
                ArrCheckColl = new string[] { "AZBW", "CZLX", "LJFS", "GGXH" };
                ArrCheckMess = new string[] { "安装部位", "材质类型", "连接方式", "规格、型号" };
                base.Parm = value;
        ...
        {
            this.bindingSource1.DataSource = InfTable.水灭火管道安装;
            this.InfTable.水灭火管道安装.RowChanged += new DataRowChangeEventHandler(this.RowChanged);
            ...
            this.GDXDCXbindingSource.DataSource = SplitDataFW(GDXDCXbindingSource, "GGXH", "MaxGGXH", "MinGGXH", '～');
            ...
            try
            {
                ...
                if (isAdd)
                {
                    ...
                    foreach (DataRowView item in this.GDAZBindingSource)
                    {
                        ...
                        str_xs.Append(string.IsNullOrEmpty(toString(drCurrent["CZBW"])) ? " and  ((BTMS='操作部位' and GD is null)" : string.Format(" and  ((BTMS='操作部位' and GD = '{0}')", drCurrent["CZBW"]))
                              .Append(string.IsNullOrEmpty(toString(drCurrent["CZGD"])) ? " or  (BTMS='操作高度' and GD is null))" : string.Format(" or  (BTMS='操作高度' and GD = '{0}'))", drCurrent["CZGD"]));
                    ...
                    #region 气压罐支吊架管网水
                    if ("制作、安装".Equals(toString(drCurrent["ZDJ"])))
                    {
                        this.QYGZDJGWSBindingSource.Filter = " XT='水灭火' and FL='支吊架'";
                    ...
                    this.QYGZDJGWSBindingSource.Filter = "";
                    this.QYGZDJGWSBindingSource.Filter = " XT='水灭火' and FL='管网水冲洗' and GGXH like'%," + drCurrent["GGXH"].ToString() + ",%'";
                    ...
                    for (int i = 0; i < ffsycx_filter.Length; i++)
                    {
                        ...
                        foreach (DataRowView item in FFSYCXBindingSource)
                        {
                            ...
                            if (!string.IsNullOrEmpty(toString(item[bw_gclxs[i]])) && (toString(item[bw_gclxs[i]]).Contains('Φ') || toString(item[bw_gclxs[i]]).Contains("HD")))
                                row["XS"] = Math.Round(ToolKit.Calculate(item[bw_gclxs[i]].ToString().Replace("Φ", toString(jszj)).Replace("HD", toString(drCurrent["BWHD"]))), 6);
                            else
                    ...
                    {
                        string strItem = string.Format("FL like '%,{0},%' and MinGGXH<={1} and MaxGGXH >= {1}", "管道压力试验", ToolKit.ParseInt(drGCZJ["GCZJ"]));
        ...
        protected override void btnRefreshQDMC_Click(object sender, EventArgs e)
        {
            ...
            if (!string.IsNullOrEmpty(toString(drCurrent["ZDJ"])))
            {
                strContent += "\r\n" + (++i) + ".支吊架：" + drCurrent["ZDJ"];
            }
            int ii = 0;
