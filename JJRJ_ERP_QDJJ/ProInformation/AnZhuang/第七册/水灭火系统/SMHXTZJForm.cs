...
{
    public partial class SMHXTZJForm : BaseUI
    {
        public SMHXTZJForm()
        {
            InitializeComponent();
        ...
        private void SMHXTZJForm_Load(object sender, EventArgs e)
        {
            OnlyOneDataSource();
        }
        ...
        public override object Parm
        {
            ...
            set
            {
                ...
                ArrCheckColl = new string[] { "CZLX", "GGXH" };
                ArrCheckMess = new string[] { "材质、类型", "规格、型号" };
                base.Parm = value;
        ...
        {
            this.bindingSource1.DataSource = InfTable.水灭火系统组件;
            this.InfTable.水灭火系统组件.RowChanged += new DataRowChangeEventHandler(this.RowChanged);

            this.XTZJBindingSource.DataSource = APP.Application.Global.DataTamp.安装专业工程信息表.Tables["系统组件"];
        ...
        private void ScreenWDBH(bool isAdd)
        {
            try
            {
                ...
                if (isAdd)
                {
                    ...
                    strString.Append(string.IsNullOrEmpty(toString(drCurrent["CZLX"])) ? " and  CZLX is null" : string.Format(" and  CZLX = '{0}'", drCurrent["CZLX"]))
                             .Append(string.IsNullOrEmpty(toString(drCurrent["GGXH"])) ? " and  GGXH is null" : string.Format("  and GGXH = '{0}'", drCurrent["GGXH"]));
                    this.XTZJBindingSource.Filter = strString.ToString();

                    foreach (DataRowView item in this.XTZJBindingSource)
                    {
                        ...
                        StringBuilder str_xs = new StringBuilder(" SYFW='消防' and BTMS='操作部位' ");
        ...
        private void gridView1_CustomRowCellEditForEditing(object sender, DevExpress.XtraGrid.Views.Grid.CustomRowCellEditEventArgs e)
        {
            ...
            switch (e.Column.FieldName)
            {
                case "CZLX":
                    ...
                    this.XTZJBindingSource.Filter = " XT='水灭火' and CZLX is not null";
                    //字符串截断并去重复
                    popControl1.DataSource = RemoveRepeat(this.XTZJBindingSource, "CZLX");

                    popControl1.ColName = new string[] { "材质、类型|CZLX|CZLX" };
                ...
                case "GGXH":
                    ...
                    strString.Append(string.IsNullOrEmpty(toString(currRow["CZLX"])) ? " and CZLX is null" : string.Format(" and  CZLX = '{0}'", currRow["CZLX"]));
                    this.XTZJBindingSource.Filter = strString.ToString();

                    popControl1.DataSource = RemoveRepeat(XTZJBindingSource, "GGXH");
        ...
        private void checkeArr()
        {
            ...
            if (currRow != null)
            {
                ...
                this.XTZJBindingSource.Filter = strString.ToString();
                if (0 < XTZJBindingSource.Count)
                {
                    strString.Append("  and GGXH is null");
                    this.XTZJBindingSource.Filter = strString.ToString();
                    if (1 > XTZJBindingSource.Count)
                    {
                        checkMess.Add("规格、型号");
                        CheckColl.Add("GGXH");
                    }
