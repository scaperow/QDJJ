...
{
    public partial class TJFZZAZForm : BaseUI
    {
        public TJFZZAZForm()
        {
            InitializeComponent();
        ...
        private void TJFZZAZForm_Load(object sender, EventArgs e)
        {
            OnlyOneDataSource();
        }
        ...
        {
            this.bindingSource1.DataSource = InfTable.调节阀制作安装;
            this.InfTable.调节阀制作安装.RowChanged += new DataRowChangeEventHandler(this.RowChanged);
        ...
        private void ScreenWDBH(bool isAdd)
        {
            try
            {
                ...
                if (isAdd)
                {
                    ...
                    StringBuilder strString = new StringBuilder(" LB = '调节阀'");
                    strString.Append(string.IsNullOrEmpty(toString(drCurrent["TJFMC"])) ? " and MC is null" : string.Format(" and MC='{0}'", drCurrent["TJFMC"]))
                             .Append(string.IsNullOrEmpty(toString(drCurrent["ZJHZCYN"])) ? " and GGXH is null" : string.Format(" and GGXH='{0}'", drCurrent["ZJHZCYN"]));
        ...
        protected override void btnRefreshQDMC_Click(object sender, EventArgs e)
        {
            ...
            int i = 0;
            if (!string.IsNullOrEmpty(drCurrent["TJFMC"].ToString()))
            {
                strContent += "\r\n" + (++i) + ".调节阀名称：" + drCurrent["TJFMC"];
            }
            if (!string.IsNullOrEmpty(drCurrent["ZJHZCYN"].ToString()))
            {
                strContent += "\r\n" + (++i) + ".直径或周长(mm)以内：" + drCurrent["ZJHZCYN"];
            }
        ...
        private void gridView1_CustomRowCellEditForEditing(object sender, DevExpress.XtraGrid.Views.Grid.CustomRowCellEditEventArgs e)
        {
            ...
            {
                case "TJFMC":
                    this.TFKTQDDEBindingSource.Filter = " LB = '调节阀' and MC is not null";
                    popControl1.DataSource = RemoveRepeat(TFKTQDDEBindingSource, "MC");

                    popControl1.ColName = new string[] { "调节阀名称|MC|TJFMC" };
                    //清除依赖项数据
                    popControl1.RemoveDefaultColName = new string[] { "ZJHZCYN" };
                    popControl1.bind();
                    break;
                case "ZJHZCYN":
                    strString = new StringBuilder(" LB = '调节阀'");
                    strString.Append(string.IsNullOrEmpty(toString(currRow["TJFMC"])) ? " and MC is null" : string.Format(" and MC='{0}'", currRow["TJFMC"]))
                             .Append(" and GGXH is not null");
                    this.TFKTQDDEBindingSource.Filter = strString.ToString();
                    popControl1.DataSource = RemoveRepeat(TFKTQDDEBindingSource, "GGXH");

                    popControl1.ColName = new string[] { "直径或周长(mm)以内|GGXH|ZJHZCYN" };
                    popControl1.bind();
                    break;
                case "CZGD":
        ...
        private void checkeArr()
        {
            ...
            if (currRow != null)
            {
                ...
                //点击确定清单前   判断必填项
                this.TFKTQDDEBindingSource.Filter = " LB = '调节阀'";
                if (0 < TFKTQDDEBindingSource.Count)
                {
                    this.TFKTQDDEBindingSource.Filter = " LB = '调节阀' and MC is null";
                ...
                if (0 < TFKTQDDEBindingSource.Count)
                {
                    ...
                    if (1 > TFKTQDDEBindingSource.Count)
                    {
                        checkMess.Add("直径或周长(mm)以内");
                        CheckColl.Add("ZJHZCYN");
                    }
                }
