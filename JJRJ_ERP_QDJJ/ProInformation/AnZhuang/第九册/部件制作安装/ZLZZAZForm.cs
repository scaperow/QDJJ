...
{
    public partial class ZLZZAZForm : BaseUI
    {
        public ZLZZAZForm()
        {
            InitializeComponent();
        ...
        private void ZLZZAZForm_Load(object sender, EventArgs e)
        {
            OnlyOneDataSource();
        }
        ...
        {
            this.bindingSource1.DataSource = InfTable.罩类制作安装;
            this.InfTable.罩类制作安装.RowChanged += new DataRowChangeEventHandler(this.RowChanged);
        ...
        private void ScreenWDBH(bool isAdd)
        {
            try
            {
                ...
                if (isAdd)
                {
                    ...
                    #region 确定清单
                    string strQDWhere = string.Format("MC like '%{0}%'", drCurrent["LX"]);
                    ...
                    StringBuilder strString = new StringBuilder(" LB = '罩类'");
                    strString.Append(string.IsNullOrEmpty(toString(drCurrent["LX"])) ? " and MC is null" : string.Format(" and MC='{0}'", drCurrent["LX"]))
        ...
        private void gridView1_CustomRowCellEditForEditing(object sender, DevExpress.XtraGrid.Views.Grid.CustomRowCellEditEventArgs e)
        {
            ...
            {
                case "LX":
                    this.TFKTQDDEBindingSource.Filter = " LB = '罩类' and MC is not null";
                    popControl1.DataSource = RemoveRepeat(TFKTQDDEBindingSource, "MC");

                    popControl1.ColName = new string[] { "材质|MC|LX" };
                    //清除依赖项数据
                    popControl1.RemoveDefaultColName = new string[] {"GG" };
                    popControl1.bind();
                    break;
                case "GG":
                    strString = new StringBuilder(" LB = '罩类'");
                    strString.Append(string.IsNullOrEmpty(toString(currRow["LX"])) ? " and MC is null" : string.Format(" and MC='{0}'", currRow["LX"]))
        ...
        private void checkeArr()
        {
            ...
            if (currRow != null)
            {
                ...
                //点击确定清单前   判断必填项
                this.TFKTQDDEBindingSource.Filter = " LB = '罩类'";
                if (0 < TFKTQDDEBindingSource.Count)
                {
                    this.TFKTQDDEBindingSource.Filter = " LB = '罩类' and MC is null";
