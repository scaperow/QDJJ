...
{
    public partial class FKZZAZForm : BaseUI
    {
        public FKZZAZForm()
        {
            InitializeComponent();
        ...
        private void FKZZAZForm_Load(object sender, EventArgs e)
        {
            OnlyOneDataSource();
        }
        ...
        {
            this.bindingSource1.DataSource = InfTable.风口制作安装;
            this.InfTable.风口制作安装.RowChanged += new DataRowChangeEventHandler(this.RowChanged);
        ...
        private void ScreenWDBH(bool isAdd)
        {
            try
            {
                ...
                if (isAdd)
                {
                    ...
                    #region 确定清单
                    string strQDWhere = string.Format("MC like '%{0}%'", drCurrent["FKMC"]);
                    ...
                    StringBuilder strString = new StringBuilder(" LB = '风口'");
                    strString.Append(string.IsNullOrEmpty(toString(drCurrent["FKMC"])) ? " and MC is null" : string.Format(" and MC='{0}'", drCurrent["FKMC"]))
        ...
        protected override void btnRefreshQDMC_Click(object sender, EventArgs e)
        {
            ...
            int i = 0;
            if (!string.IsNullOrEmpty(drCurrent["FKMC"].ToString()))
            {
                strContent += "\r\n" + (++i) + ".风口名称：" +"风口"+ drCurrent["FKMC"];
            }
        ...
        private void gridView1_CustomRowCellEditForEditing(object sender, DevExpress.XtraGrid.Views.Grid.CustomRowCellEditEventArgs e)
        {
            ...
            {
                case "FKMC":
                    this.TFKTQDDEBindingSource.Filter = " LB = '风口' and MC is not null";
                    popControl1.DataSource = RemoveRepeat(TFKTQDDEBindingSource, "MC");

                    popControl1.ColName = new string[] { "风口名称|MC|FKMC" };
                    //清除依赖项数据
                    popControl1.RemoveDefaultColName = new string[] { "LX" ,"GGXH"};
                    popControl1.bind();
                    break;
                case "LX":
                    strString = new StringBuilder(" LB = '风口'");
                    strString.Append(string.IsNullOrEmpty(toString(currRow["FKMC"])) ? " and MC is null" : string.Format(" and MC='{0}'", currRow["FKMC"]))
        ...
        private void checkeArr()
        {
            ...
            if (currRow != null)
            {
                ...
                //点击确定清单前   判断必填项
                this.TFKTQDDEBindingSource.Filter = " LB = '风口'";
                if (0 < TFKTQDDEBindingSource.Count)
                {
                    this.TFKTQDDEBindingSource.Filter = " LB = '风口' and MC is null";
