...
{
    public partial class XSQJYXForm : BaseUI
    {
        public XSQJYXForm()
        {
            InitializeComponent();
        ...
        private void XSQJYXForm_Load(object sender, EventArgs e)
        {
            OnlyOneDataSource();
        }
        ...
        {
            this.bindingSource1.DataSource = InfTable.消声器静压箱;
            this.InfTable.消声器静压箱.RowChanged += new DataRowChangeEventHandler(this.RowChanged);
        ...
        private void ScreenWDBH(bool isAdd)
        {
            try
            {
                ...
                if (isAdd)
                {
                    ...
                    #region 确定清单
                    string strQDWhere = string.Format("MC like '%{0}%'", drCurrent["FL"]);
                    ...
                    StringBuilder strString = new StringBuilder(" LB = '消声器、静压箱'");
                    strString.Append(string.IsNullOrEmpty(toString(drCurrent["FL"])) ? " and MC is null" : string.Format(" and MC='{0}'", drCurrent["FL"]))
                             .Append(string.IsNullOrEmpty(toString(drCurrent["LB"])) ? " and LX is null" : string.Format(" and LX='{0}'", drCurrent["LB"]))
        ...
        private void gridView1_CustomRowCellEditForEditing(object sender, DevExpress.XtraGrid.Views.Grid.CustomRowCellEditEventArgs e)
        {
            ...
            {
                case "FL":
                    this.TFKTQDDEBindingSource.Filter = " LB = '消声器、静压箱' and MC is not null";
                    popControl1.DataSource = RemoveRepeat(TFKTQDDEBindingSource, "MC");

                    popControl1.ColName = new string[] { "分类|MC|FL" };
                    //清除依赖项数据
                    popControl1.RemoveDefaultColName = new string[] { "LB", "CCGG" };
                    popControl1.bind();
                    break;
                case "LB":
                    strString = new StringBuilder(" LB = '消声器、静压箱'");
                    strString.Append(string.IsNullOrEmpty(toString(currRow["FL"])) ? " and MC is null" : string.Format(" and MC='{0}'", currRow["FL"]))
                             .Append(" and LX is not null");
                    this.TFKTQDDEBindingSource.Filter = strString.ToString();
                    popControl1.DataSource = RemoveRepeat(TFKTQDDEBindingSource, "LX");

                    popControl1.ColName = new string[] { "类别|LX|LB" };
                    //清除依赖项数据
                ...
                case "CCGG":
                    strString = new StringBuilder(" LB = '消声器、静压箱'");
                    strString.Append(string.IsNullOrEmpty(toString(currRow["FL"])) ? " and MC is null" : string.Format(" and MC='{0}'", currRow["FL"]))
                             .Append(string.IsNullOrEmpty(toString(currRow["LB"])) ? " and LX is null" : string.Format(" and LX='{0}'", currRow["LB"]))
        ...
        private void checkeArr()
        {
            ...
            if (currRow != null)
            {
                ...
                //点击确定清单前   判断必填项
                this.TFKTQDDEBindingSource.Filter = " LB = '消声器、静压箱'";
                if (0 < TFKTQDDEBindingSource.Count)
                {
                    this.TFKTQDDEBindingSource.Filter = " LB = '消声器、静压箱' and MC is null";
