...
{
    public partial class GNQJForm : BaseUI
    {
        public GNQJForm()
        {
            InitializeComponent();
        ...
        private void GNQJForm_Load(object sender, EventArgs e)
        {
            OnlyOneDataSource();
        }
        ...
        {
            this.bindingSource1.DataSource = InfTable.采暖热源供暖器具;
            this.InfTable.采暖热源供暖器具.RowChanged += new DataRowChangeEventHandler(this.RowChanged);
        ...
        private void ScreenWDBH(bool isAdd)
        {
            try
            {
                ...
                if (isAdd)
                {
                    ...
                    string strDEWhere = string.Format(" LB='供暖器具' and FL='{0}' and GGXH='{1}' and (GCZJ = '{2}' or GCZJ is null)"
                                                , drCurrent["FL"], drCurrent["GGXH"], drCurrent["GCZJ"]);
        ...
        private void gridView1_CustomRowCellEditForEditing(object sender, DevExpress.XtraGrid.Views.Grid.CustomRowCellEditEventArgs e)
        {
            ...
            switch (e.Column.FieldName)
            {
                case "FL":
                    ...
                    QJQDDEBindingSource.Filter = " LB='供暖器具' and FL is not null";
                    ...
                    //清除依赖项数据
                    popControl1.RemoveDefaultColName = new string[] { "GGXH", "GCZJ" };
                    popControl1.bind();
                    break;
                case "GGXH":

                    this.QJQDDEBindingSource.Filter = string.Format(" LB = '供暖器具' and FL='{0}' and GGXH is not null", toString(currRow["FL"]));
                    DataRowView ggxh = QJQDDEBindingSource.Current as DataRowView;
                    popControl1.DataSource = RemoveRepeat(ggxh.DataView.Table, "GGXH");

                    popControl1.ColName = new string[] { "分类|GGXH|GGXH" };
                    //清除依赖项数据
                ...
                case "GCZJ":
                    ...
                    QJQDDEBindingSource.Filter = string.Format(" LB = '供暖器具' and FL='{0}' and GGXH='{1}' and GCZJ is not null", toString(currRow["FL"]), toString(currRow["GGXH"]));

                    popControl1.ColName = new string[] { "分类|GCZJ|GCZJ" };
                    popControl1.bind();
                    break;
                case "AZGD":

                    popControl1.DataSource = this.GDSZBindingSource;
                    this.GDSZBindingSource.Filter = " SYFW = '给排水' and BTMS ='操作高度' and GD is not null";
        ...
        private void checkeArr()
        {
            ...
            if (currRow != null)
            {
                ...
                CheckColl.Add("GGXH");
                //点击确定清单前   判断必填项
                QJQDDEBindingSource.Filter = string.Format(" LB = '供暖器具' and FL='{0}' and GGXH='{1}'", toString(currRow["FL"]), toString(currRow["GGXH"]));

                if (0 < QJQDDEBindingSource.Count)
                {
                    QJQDDEBindingSource.Filter = string.Format(" LB = '供暖器具' and FL='{0}' and GGXH='{1}' and GCZJ is null ", toString(currRow["FL"]), toString(currRow["GGXH"]));
                    if (1 > QJQDDEBindingSource.Count)
                    {
                        checkMess.Add("公称直径");
                        CheckColl.Add("GCZJ");
                    }
