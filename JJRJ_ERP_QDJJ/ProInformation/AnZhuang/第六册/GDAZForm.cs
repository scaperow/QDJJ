...
{
    public partial class GDAZForm : BaseUI
    {
        public GDAZForm()
        {
            InitializeComponent();
        ...
        private void GDAZForm_Load(object sender, EventArgs e)
        {
            OnlyOneDataSource(); 
        }
        ...
        {
            this.bindingSource1.DataSource = InfTable.管道安装;
            this.InfTable.管道安装.RowChanged += new DataRowChangeEventHandler(this.RowChanged);
        ...
        private void ScreenWDBH(bool isAdd)
        {
            try
            {
                ...
                if (isAdd)
                {
                    ...
                    if (drvGczj != null)
                    {
                        gczj = string.IsNullOrEmpty(toString(drvGczj["GCZJ"])) ? -1 : decimal.Parse(toString(drvGczj["GCZJ"]));
                        jszj = string.IsNullOrEmpty(toString(drvGczj["JSZJ"])) ? 0 : decimal.Parse(toString(drvGczj["JSZJ"]));
                    }
                    else
                    {
                        gczj = -1;
                    ...
                    if (gczj != -1)
                    {
                        ...
                        strString.Append(" or ( JRHD is null ")
                                 .Append(jszj == 0 ? " and  MaxSBZJ is null" : string.Format(" and  MaxSBZJ>={0}", jszj))
                                 .Append(jszj == 0 ? "  and MinSBZJ is null)" : string.Format(" and MinSBZJ<={0})", jszj));
                        strString.Append(" or ( SBZJ is null and JRHD is not null)");

                        strString.Append(jszj == 0 ? " or (MaxSBZJ is null" : string.Format(" or (MaxSBZJ>={0}", jszj))
                                 .Append(jszj == 0 ? "  and MinSBZJ is null))" : string.Format(" and MinSBZJ<={0}))", jszj));
                    ...
                    #region 工业管道确定定额
                    strString = new StringBuilder(" LB='管道安装' ");
                    ...
                    DataRowView drv = this.GYGDQDDEBindingSource.Current as DataRowView;
                    ...
                    strString = new StringBuilder(" FL='除锈工程' and BW='管道'");
                    ...
                    for (int i = 0; i < ffsycx_filter.Length; i++)
                    {
                        ...
                        foreach (DataRowView item in FFSYCXBindingSource)
                        {
                            ...
                            row["DEMC"] = item["DEMC"];
                            row["DW"] = item["DEDW"];
                            if (toString(item[bw_gclxs[i]]).Contains('Φ'))
                                row["XS"] = Math.Round(ToolKit.Calculate(toString(item[bw_gclxs[i]]).Replace("Φ", toString(jszj)).Replace("HD", toString(drCurrent["BWHD"]))), 6);
                            else
        ...
        private void gridView1_CustomRowCellEditForEditing(object sender, DevExpress.XtraGrid.Views.Grid.CustomRowCellEditEventArgs e)
        {
            ...
            {
                case "FL":
                    strString = new StringBuilder(" LB='管道安装' and FL is not null");
                    GYGDQDDEBindingSource.Filter = strString.ToString();
                ...
                case "CZ":
                    ...
                    strString = new StringBuilder(" LB='管道安装' and CZ is not null");
                ...
                case "LJFS":
                    ...
                    strString = new StringBuilder(" LB='管道安装' and LJFS is not null");
                    strString.Append(string.IsNullOrEmpty(toString(currRow["FL"])) ? " and FL is null" : string.Format(" and  FL = '{0}'", currRow["FL"]))
                ...
                case "GJ":
                    ...
                    strString = new StringBuilder(" LB='管道安装' and GJ is not null");
                    strString.Append(string.IsNullOrEmpty(toString(currRow["FL"])) ? " and  FL is null" : string.Format(" and  FL = '{0}'", currRow["FL"]))
                             .Append(string.IsNullOrEmpty(toString(currRow["CZ"])) ? " and  CZ is null" : string.Format("  and CZ = '{0}'", currRow["CZ"]))
                    ...
                    break;
                case "CX":
                    this.FFSYCXBindingSource.Filter = string.Format("FL ='除锈工程' and BW ='管道' and LX is not null");
