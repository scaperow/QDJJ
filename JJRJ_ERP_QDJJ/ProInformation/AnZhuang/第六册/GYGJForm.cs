...
{
    public partial class GYGJForm : BaseUI
    {
        public GYGJForm()
        {
            InitializeComponent();
        ...
        private void GYGJForm_Load(object sender, EventArgs e)
        {
            OnlyOneDataSource();
        }
        ...
        {
            this.bindingSource1.DataSource = InfTable.管件;
            this.InfTable.管件.RowChanged += new DataRowChangeEventHandler(this.RowChanged);
        ...
        private void ScreenWDBH(bool isAdd)
        {
            try
            {
                ...
                if (isAdd)
                {
                    ...
                    #region 工业管道确定定额
                    strString = new StringBuilder(" LB='管件' ");
        ...
        private void checkeArr()
        {
            ...
            if (currRow != null)
            {
                ...
                checkMess.Add("材质");
                CheckColl.Add("CZ");
                StringBuilder strString = new StringBuilder(" LB='管件' ");
                strString.Append(string.IsNullOrEmpty(toString(currRow["FL"])) ? " and FL is null" : string.Format(" and  FL = '{0}'", currRow["FL"]))
                         .Append(string.IsNullOrEmpty(toString(currRow["CZ"])) ? " and  CZ is null" : string.Format("  and CZ = '{0}'", currRow["CZ"]));
