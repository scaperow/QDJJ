...
{
    public partial class QTGDForm : BaseUI
    {
        public QTGDForm()
        {
            InitializeComponent();
        ...
        private void QTGDForm_Load(object sender, EventArgs e)
        {
            OnlyOneDataSource();
        }
        ...
        {
            this.bindingSource1.DataSource = InfTable.其他管道;
            this.InfTable.其他管道.RowChanged += new DataRowChangeEventHandler(this.RowChanged);
        ...
        private void ScreenWDBH(bool isAdd)
        {
            try
            {
                ...
                if (isAdd)
                {
                    ...
                    StringBuilder strString = new StringBuilder(" LB = '其他管道'");
                    strString.Append(string.IsNullOrEmpty(toString(drCurrent["FL"])) ? " and FL is null" : string.Format(" and FL='{0}'", drCurrent["FL"]))
                             .Append(string.IsNullOrEmpty(toString(drCurrent["GCZJYN"])) ? " and GJ is null" : string.Format(" and GJ = '{0}'", drCurrent["GCZJYN"]));
        ...
        private void gridView1_CustomRowCellEditForEditing(object sender, DevExpress.XtraGrid.Views.Grid.CustomRowCellEditEventArgs e)
        {
            ...
            {
                case "FL":
                    this.GYGDQDDEBindingSource.Filter = " LB = '其他管道' and FL is not null";
                    ...
                    break;
                case "GCZJYN":
                    strString = new StringBuilder(" LB = '其他管道' and GJ is not null");
                    ...
                    popControl1.ColName = new string[] { "公称直径(mm)以内|GJ|GCZJYN" };
                    popControl1.bind();
                    break;
            }
        }
        ...
        private void checkeArr()
        {
            ...
            if (currRow != null)
            {
                ...
                //点击确定清单前   判断必填项
                this.GYGDQDDEBindingSource.Filter = " LB = '其他管道'";
                if (0 < GYGDQDDEBindingSource.Count)
                {
                    this.GYGDQDDEBindingSource.Filter = " LB = '其他管道' and FL is null";
                ...
                strString = new StringBuilder(" LB = '其他管道'");
                strString.Append(string.IsNullOrEmpty(toString(currRow["FL"])) ? " and FL is null" : string.Format(" and FL='{0}'", currRow["FL"]));
                GYGDQDDEBindingSource.Filter = strString.ToString();
                if (0 < GYGDQDDEBindingSource.Count)
                {
                    strString.Append(" and GJ is null");
                    this.GYGDQDDEBindingSource.Filter = strString.ToString();
                    if (1 > GYGDQDDEBindingSource.Count)
                    {
