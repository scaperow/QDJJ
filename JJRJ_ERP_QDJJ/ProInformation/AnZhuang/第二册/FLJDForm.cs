...
{
    public partial class FLJDForm : BaseUI
    {
        public FLJDForm()
        {
            InitializeComponent();
        ...
        private void FLJDForm_Load(object sender, EventArgs e)
        {
            OnlyOneDataSource();
        }
        ...
        {
            this.bindingSource1.DataSource = InfTable.防雷接地;
            this.InfTable.防雷接地.RowChanged += new DataRowChangeEventHandler(this.RowChanged);
            ...
            try
            {
                ...
                if (isAdd)
                {
                    ...
                    strString.Append(string.IsNullOrEmpty(toString(drCurrent["FL"])) ? " and FL is null" : string.Format(" and FL like '%,{0},%'", drCurrent["FL"]))
                             .Append(string.IsNullOrEmpty(toString(drCurrent["JDJB"])) ? " and GGXH is null" : string.Format(" and GGXH like '%,{0},%'", drCurrent["JDJB"]));
                    strFilter[0] = strString.ToString();

                    strString = new StringBuilder("  LB = '防雷接地' and LX ='接地母线'");
                    strString.Append(string.IsNullOrEmpty(toString(drCurrent["FL"])) ? " and FL is null" : string.Format(" and FL like '%,{0},%'", drCurrent["FL"]))
                             .Append(string.IsNullOrEmpty(toString(drCurrent["JDMX"])) ? " and GGXH is null" : string.Format(" and GGXH like '%,{0},%'", drCurrent["JDMX"]));
                    strFilter[1] = strString.ToString();

                    strString = new StringBuilder("  LB = '防雷接地' and LX ='接地跨接线'");
                    strString.Append(string.IsNullOrEmpty(toString(drCurrent["FL"])) ? " and FL is null" : string.Format(" and FL like '%,{0},%'", drCurrent["FL"]))
                             .Append(string.IsNullOrEmpty(toString(drCurrent["JDKJX"])) ? " and GGXH is null" : string.Format(" and GGXH like '%,{0},%'", drCurrent["JDKJX"]));
                    strFilter[2] = strString.ToString();

                    strString = new StringBuilder("  LB = '防雷接地' and LX ='避雷针（网）'");
                    strString.Append(string.IsNullOrEmpty(toString(drCurrent["FL"])) ? " and FL is null" : string.Format(" and FL like '%,{0},%'", drCurrent["FL"]))
                             .Append(string.IsNullOrEmpty(toString(drCurrent["BLZW"])) ? " and GGXH is null" : string.Format(" and GGXH like '%,{0},%'", drCurrent["BLZW"]));
                    strFilter[3] = strString.ToString();

                    strString = new StringBuilder("  LB = '防雷接地' and LX ='避雷引下线'");
                    strString.Append(string.IsNullOrEmpty(toString(drCurrent["FL"])) ? " and FL is null" : string.Format(" and FL like '%,{0},%'", drCurrent["FL"]))
                             .Append(string.IsNullOrEmpty(toString(drCurrent["BLYXX"])) ? " and GGXH is null" : string.Format(" and GGXH like '%,{0},%'", drCurrent["BLYXX"]));
                    strFilter[4] = strString.ToString();

                    strString = new StringBuilder("  LB = '防雷接地' and LX ='避雷网'");
                    strString.Append(string.IsNullOrEmpty(toString(drCurrent["FL"])) ? " and FL is null" : string.Format(" and FL like '%,{0},%'", drCurrent["FL"]))
                             .Append(string.IsNullOrEmpty(toString(drCurrent["BLW"])) ? " and GGXH is null" : string.Format(" and GGXH like '%,{0},%'", drCurrent["BLW"]));
                    strFilter[5] = strString.ToString();

                    strString = new StringBuilder("  LB = '防雷接地' and LX ='等电位联结'");
                    strString.Append(string.IsNullOrEmpty(toString(drCurrent["FL"])) ? " and FL is null" : string.Format(" and FL like '%,{0},%'", drCurrent["FL"]))
                             .Append(string.IsNullOrEmpty(toString(drCurrent["DDWLJ"])) ? " and GGXH is null" : string.Format(" and GGXH like '%,{0},%'", drCurrent["DDWLJ"]));
                    strFilter[6] = strString.ToString();

                    strString = new StringBuilder("  LB = '防雷接地' and LX ='油漆（防腐）'");
                    strString.Append(string.IsNullOrEmpty(toString(drCurrent["FL"])) ? " and FL is null" : string.Format(" and FL like '%,{0},%'", drCurrent["FL"]))
                             .Append(string.IsNullOrEmpty(toString(drCurrent["YQFF"])) ? " and GGXH is null" : string.Format(" and GGXH like '%,{0},%'", drCurrent["YQFF"]));
                    strFilter[7] = strString.ToString();
        ...
        protected override void btnRefreshQDMC_Click(object sender, EventArgs e)
        {
            ...
            {
                strContent += "\r\n" + (++i) + ".防雷接地分类：" + drCurrent["FL"];
            }
            if (!string.IsNullOrEmpty(drCurrent["JDJB"].ToString()))
            {
                strContent += "\r\n" + (++i) + ".接地极（板）：" + drCurrent["JDJB"];
            }
            if (!string.IsNullOrEmpty(drCurrent["JDMX"].ToString()))
            {
                strContent += "\r\n" + (++i) + ".接地母线：" + drCurrent["JDMX"];
            }
            if (!string.IsNullOrEmpty(drCurrent["JDKJX"].ToString()))
            {
                strContent += "\r\n" + (++i) + ".接地跨接线：" + drCurrent["JDKJX"];
            }
            if (!string.IsNullOrEmpty(drCurrent["BLZW"].ToString()))
            {
                strContent += "\r\n" + (++i) + ".避雷针（网）：" + drCurrent["BLZW"];
            }
            if (!string.IsNullOrEmpty(drCurrent["BLYXX"].ToString()))
            {
                strContent += "\r\n" + (++i) + ".避雷引下线：" + drCurrent["BLYXX"];
            }
            if (!string.IsNullOrEmpty(drCurrent["BLW"].ToString()))
            {
                strContent += "\r\n" + (++i) + ".避雷网：" + drCurrent["BLW"];
            }
            if (!string.IsNullOrEmpty(drCurrent["DDWLJ"].ToString()))
            {
                strContent += "\r\n" + (++i) + ".等电位联结：" + drCurrent["DDWLJ"];
            }
            if (!string.IsNullOrEmpty(drCurrent["YQFF"].ToString()))
            {
                strContent += "\r\n" + (++i) + ".油漆（防腐）：" + drCurrent["YQFF"];
            }
        ...
        private void checkeArr()
        {
            ...
            if (currRow != null)
            {
                ...
                //点击确定清单前   判断必填项 
                this.DQQDDEBindingSource.Filter = " LB = '防雷接地'";
                if (0 < DQQDDEBindingSource.Count)
                {
                    this.DQQDDEBindingSource.Filter = " LB = '防雷接地' and FL is null";
